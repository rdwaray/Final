...
Feature: User Registration
  ...
  Scenario: Successfully register a new user with valid details
    Given I provide a valid email "eve.holt@reqres.in" and a valid password "pistol"
    When  I submit the registration request
    And the response register code should be 200
#    Then the response should contain the newly created user ID
 ...
 # Scenario: Attempt to register with an invalid email format
   ...
   # When I submit the registration request
   # And the response faliure register code should be 400
  ...
  Scenario: Attempt to register with a missing password
    Given I provide a valid email "sydney@fife" and an empty password
    When I submit the registration request
    And the response faliure register code should be 400
...
#Note pengetesan nya hanya bisa register email dan register with missing password di website reqres.in